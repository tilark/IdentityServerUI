  ...
  <h2>管理Client</h2>
   <!--查询条件-->
    <el-form :inline="true" :model="searchForm" ref="searchForm" label-position="left">
      ...
      <el-form-item label="GrantType">
        <el-select v-model="searchForm.allowedGrantType">
          <el-option
            v-for="item in allowedGrantTypes"
            :key="item.value"
            :label="item.label"
            :value="item.value">
          </el-option>
...
export default{
  ...
  data() {
    return {
      ...
      listLoading: false,
      clientList: [],
      allowedGrantTypes: [{ 'label': 'Impilict', 'value': 'Impilict' }, { 'label': 'ClientCredentials', 'value': 'ClientCredentials' }]
    }
  },
  methods: {
    getClientList() {
      this.$refs.searchForm.validate(valid => {
        if (valid) {
          ...
          getClients(this.searchForm).then(response => {
